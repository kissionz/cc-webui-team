...
export interface LineageGraph { rootId: string; scope: string; direction: string; tables: LineageTable[]; edges: LineageEdge[]; truncated: boolean; pathReversed?: boolean; pathFound?: boolean }
export type LineageSyncStage = "scope" | "metadata" | "tasks" | "lineage";
export interface LineageSyncRun { id: string; trigger: "schedule" | "manual"; requestedBy?: string | null; dataDate: string; status: "running" | "success" | "failed"; currentStage: LineageSyncStage; projectsProcessed: number; tablesProcessed: number; columnsProcessed: number; tasksStaged: number; jobsProcessed: number; edgesProcessed: number; error?: string | null; startedAt: number; progressUpdatedAt: number; completedAt?: number | null }
