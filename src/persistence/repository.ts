...
export class PersistenceRepository {
  ...
  saveLineageSyncRun(run: LineageSyncRun): void {
    this.database.prepare(`INSERT INTO lineage_sync_runs(id, trigger_type, requested_by, data_date, status,
      current_stage, projects_processed, tables_processed, columns_processed, tasks_staged, jobs_processed,
      edges_processed, error, started_at, progress_updated_at, completed_at)
      VALUES (@id, @trigger, @requestedBy, @dataDate, @status, @currentStage, @projectsProcessed, @tablesProcessed,
        @columnsProcessed, @tasksStaged, @jobsProcessed, @edgesProcessed, @error, @startedAt, @progressUpdatedAt, @completedAt)
      ON CONFLICT(id) DO UPDATE SET status=excluded.status, current_stage=excluded.current_stage,
        projects_processed=excluded.projects_processed, tables_processed=excluded.tables_processed,
        columns_processed=excluded.columns_processed, tasks_staged=excluded.tasks_staged,
        jobs_processed=excluded.jobs_processed, edges_processed=excluded.edges_processed,
        error=excluded.error, progress_updated_at=excluded.progress_updated_at,
        completed_at=excluded.completed_at`).run(run);
  }
...
function lineageSyncRunFromRow(r: Row): LineageSyncRun { return { id:str(r.id!), trigger:str(r.trigger_type!) as LineageSyncRun["trigger"], requestedBy:nullableStr(r.requested_by!), dataDate:str(r.data_date!), status:str(r.status!) as LineageSyncRun["status"], currentStage:str(r.current_stage!) as LineageSyncRun["currentStage"], projectsProcessed:num(r.projects_processed!), tablesProcessed:num(r.tables_processed!), columnsProcessed:num(r.columns_processed!), tasksStaged:num(r.tasks_staged!), jobsProcessed:num(r.jobs_processed!), edgesProcessed:num(r.edges_processed!), error:nullableStr(r.error!), startedAt:num(r.started_at!), progressUpdatedAt:num(r.progress_updated_at!), completedAt:nullableNum(r.completed_at!) }; }
